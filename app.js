    ...
    for (const [k, v] of Object.entries(obj)) {
        if (typeof v === 'string') {
            ...
            for (let i = 0; i < v.length; i++) {
                if (typeof v[i] === 'string') v[i] = sanitizeString(v[i]);
                else if (v[i] && typeof v[i] === 'object') deepSanitizeInPlace(v[i]);
            }
...
function sanitizerMiddleware(req, _res, next) {
    if (req.query) deepSanitizeInPlace(req.query);
    if (req.body) deepSanitizeInPlace(req.body);
    if (req.params) deepSanitizeInPlace(req.params);
    next();
}
...
function createApp() {
    ...
    app.use(
        cors({
            origin: ['http://localhost:4174', 'https://calvaz-prime.onrender.com'],
            credentials: true,
